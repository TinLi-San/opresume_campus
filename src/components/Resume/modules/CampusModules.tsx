...
import type { JsonEducation, JsonWork, JsonProject, JsonSkill, JsonAward } from '@/types/json-resume';
...
function formatCampusDate(value: string | undefined, presentLabel: string): string {
  if (!value) return '';
...
function TimeRange({ startDate, endDate }: { startDate?: string; endDate?: string }) {
  const { t } = useTranslation();
  if (!startDate && !endDate) return null;
  const display = (v?: string) => formatCampusDate(v, t('field.present'));
  return (
          ...
          <div key={skill['x-op-id'] ?? i} className="campus-item campus-bullet" data-item-index={offset + i}>
            ...
            <span className="campus-bullet-text">
              {skill.keywords?.length ? (
                <>
                  <span className="campus-bold">{skill.name}{t('common.labelSeparator')}</span>
                  {skill.keywords.join(t('common.listSeparator'))}
                </>
              ) : (
                <>
                  {skill.name}
        ...
        {list.map((award, i) => (
          <div key={award['x-op-id'] ?? i} className="campus-item campus-bullet" data-item-index={offset + i}>
            ...
            <span className="campus-bullet-text">
              ...
              {award.date && <span className="campus-muted"> ({formatCampusDate(award.date, t('field.present'))})</span>}
            </span>
          </div>
        ))}
      </section>
